const NATWISS_CHAPTER8_TOPICS = [
  {
    ...
    quiz: [
      { id: "q1", question: "Was besagt der 1. Hauptsatz der Thermodynamik?", options: ["Entropie nimmt stets zu", "Wärme fließt stets von warm nach kalt", "Energie ist in thermodynamischen Systemen erhalten: ΔU = Q + W", "Kein Prozess hat 100% Wirkungsgrad"], correctIndex: 2, explanation: "Der 1. Hauptsatz ist der Energieerhaltungssatz: ΔU = Q + W, die innere Energie ändert sich durch Wärme und Arbeit." },
      { id: "q2", question: "Was ist die innere Energie eines idealen Gases?", options: ["Nur die potenzielle Energie", "Nur die kinetische Energie der Schwerpunktbewegung", "Die Gravitationsenergie der Gasmasse", "Die Summe aller mikroskopischen kinetischen Energien der Moleküle"], correctIndex: 3, explanation: "Die innere Energie ist die Summe aller mikroskopischen kinetischen (und potenziellen) Energien der Moleküle; beim idealen Gas nur kinetische Anteile." },
      { id: "q3", question: "Welche Wärmekapazität ist größer: C_p oder C_V?", options: ["Sie sind gleich", "C_p ist größer", "C_V ist größer", "Das hängt vom Gas ab"], correctIndex: 1, explanation: "C_p > C_V immer, weil bei konstantem Druck zusätzlich Volumenarbeit geleistet werden muss: C_p = C_V + R." },
      { id: "q4", question: "Ein System gibt Wärme ab (Q < 0) und empfängt Arbeit (W > 0). Was passiert mit U?", options: ["U sinkt immer", "ΔU = Q + W, das Ergebnis hängt von den Beträgen ab", "U bleibt konstant", "U steigt immer"], correctIndex: 1, explanation: "ΔU = Q + W: Q ist negativ, W ist positiv. Das Vorzeichen von ΔU hängt davon ab, welcher Term überwiegt." },
      { id: "q5", question: "Wie viele Freiheitsgrade hat ein zweiatomiges Gasmolekül bei Raumtemperatur?", options: ["5", "2", "6", "3"], correctIndex: 0, explanation: "Zweiatomige Moleküle haben 3 Translationsfreiheitsgrade + 2 Rotationsfreiheitsgrade = 5 Freiheitsgrade bei Raumtemperatur." },
      { id: "yn1", type: "yesno", question: "Hängt die innere Energie eines idealen Gases von seinem Volumen ab, solange die Temperatur konstant bleibt?", correctAnswer: false, reasons: ["Nein, weil beim idealen Gas U = n·C_V·T nur von der Temperatur abhängt; bei konstanter Temperatur bleibt U unabhängig vom Volumen unverändert.", "Ja, weil sich bei größerem Volumen mehr Teilchen im System befinden und U dadurch automatisch steigt.", "Nein, weil die innere Energie beim idealen Gas grundsätzlich null ist.", "Ja, weil eine Volumenänderung stets mit einer Druckänderung einhergeht, die U direkt beeinflusst."], correctReasonIndex: 0, explanation: "Für ein ideales Gas gilt U = n·C_V·T – die innere Energie hängt ausschließlich von der Temperatur ab, nicht vom Volumen oder Druck, da im idealen Gas keine Wechselwirkungsenergie zwischen den Teilchen existiert." },
      ...
      { id: "q8", question: "Was passiert bei einem isochoren Prozess?", options: ["T = const, ΔU = 0", "Q = 0, ΔU = W", "p = const, ΔU = Q + W", "V = const, W = 0, ΔU = Q"], correctIndex: 3, explanation: "Isochor = konstantes Volumen → keine Volumenarbeit (W = 0) → ΔU = Q." },
      { id: "q9", question: "Welchen Wert hat der Adiabatenexponent γ für Luft (zweiatomig, N₂ + O₂) ungefähr?", options: ["1,4", "1,2", "1,0", "1,67"], correctIndex: 0, explanation: "Für zweiatomige Gase: γ = C_p/C_V = (7/2·R)/(5/2·R) = 7/5 = 1,4. Das gilt für N₂ und O₂, die Hauptbestandteile der Luft." },
      { id: "q10", question: "Wie ändert sich die innere Energie eines idealen Gases bei isothermer Expansion?", options: ["Sie nimmt zu", "Sie hängt vom Druck ab", "Sie bleibt konstant (ΔU = 0)", "Sie nimmt ab"], correctIndex: 2, explanation: "Beim idealen Gas hängt U nur von T ab: U = n·C_V·T. Bei T = const gilt ΔU = 0." }
    ]
  },
  {
    ...
    quiz: [
      { id: "q1", question: "Bei welchem Prozess bleibt die Temperatur konstant?", options: ["Isochor", "Adiabatisch", "Isotherm", "Isobar"], correctIndex: 2, explanation: "Isotherm bedeutet T = konstant." },
      { id: "q2", question: "Was gilt für einen isochoren Prozess?", options: ["Q = 0", "V = const, W = 0", "T = const", "p = const"], correctIndex: 1, explanation: "Isochor: V = konstant → keine Volumenarbeit (W = 0), daher gilt ΔU = Q." },
      { id: "q3", question: "Poisson-Gleichung gilt für welchen Prozess?", options: ["Isotherme: p·V = const", "Isochore: V = const", "Isobare: p = const", "Adiabatische: p·Vᵞ = const"], correctIndex: 3, explanation: "Die Poisson-Gleichung p·V^γ = const beschreibt die Adiabate (Q = 0 Prozess)." },
      { id: "q4", question: "Was passiert mit der Temperatur bei adiabatischer Expansion?", options: ["T sinkt", "T hängt nur vom Druck ab", "T bleibt konstant", "T steigt"], correctIndex: 0, explanation: "Bei adiabatischer Expansion (Q = 0) muss das Gas die Arbeit aus seiner inneren Energie aufbringen → T sinkt (ΔU = W < 0)." },
      { id: "yn1", type: "yesno", question: "Ist bei einer adiabatischen Zustandsänderung eines idealen Gases die Arbeit W stets gleich der Änderung der inneren Energie ΔU?", correctAnswer: true, reasons: ["Ja, weil bei einem adiabatischen Prozess per Definition Q = 0 gilt und aus dem 1. Hauptsatz ΔU = Q + W somit direkt ΔU = W folgt.", "Nein, weil bei jeder Expansion zusätzlich Wärme mit der Umgebung ausgetauscht wird, die berücksichtigt werden muss.", "Ja, weil bei adiabatischen Prozessen die Temperatur konstant bleibt und daher ΔU immer gleich der Arbeit ist.", "Nein, weil ΔU bei adiabatischen Prozessen stets null ist, unabhängig von der geleisteten Arbeit."], correctReasonIndex: 0, explanation: "Ein adiabatischer Prozess ist per Definition durch Q = 0 charakterisiert; aus dem 1. Hauptsatz ΔU = Q + W folgt damit unmittelbar ΔU = W – die geleistete oder aufgenommene Arbeit äußert sich vollständig als Änderung der inneren Energie." },
      { id: "q5", question: "Was ist die Arbeit bei einem isochoren Prozess?", options: ["W = Q", "W = 0", "W = p·ΔV", "W = n·R·T·ln(V₂/V₁)"], correctIndex: 1, explanation: "Isochor: ΔV = 0 → W = −p·ΔV = 0. Keine Volumenarbeit." },
      { id: "q6", question: "Ein Gas expandiert isotherm. Wie ändert sich die innere Energie?", options: ["Sie steigt proportional zu V", "Sie bleibt konstant (ΔU = 0)", "Sie sinkt auf null", "Sie steigt um Q"], correctIndex: 1, explanation: "Bei idealem Gas: U = n·C_V·T. Isotherm: T = const → ΔU = 0." },
      { id: "q7", question: "Was ist W_isotherm für 1 mol bei T = 400 K, V₂/V₁ = e (Euler)?", options: ["400 J", "−400 J", "R·400·ln(e) = 400R ≈ 3326 J", "0 J"], correctIndex: 2, explanation: "W = −n·R·T·ln(V₂/V₁) = −1·R·400·ln(e) = −400R ≈ −3326 J. Das Gas leistet 3326 J Arbeit (Vorzeichen: Gas gibt Arbeit ab)." },
      { id: "q8", question: "Bei welchem Prozess nimmt das Gas keine Wärme auf oder ab?", options: ["Adiabatisch", "Isochor", "Isobar", "Isotherm"], correctIndex: 0, explanation: "Adiabatisch: Q = 0 (kein Wärmeaustausch). Dann gilt ΔU = W." },
      { id: "q9", question: "Wie lautet die Bedingung für isobare Zustandsänderung beim idealen Gas?", options: ["V/T = const", "p/T = const", "p·V = const", "V = const"], correctIndex: 0, explanation: "Isobar (p = const): aus pV = nRT folgt V/T = nR/p = const (Charles'sches Gesetz)." },
      { id: "q10", question: "Welcher Prozess ist in einem gut isolierten Behälter (Thermosflasche) am besten beschrieben?", options: ["Isobare Zustandsänderung", "Isochore Zustandsänderung", "Isotherme Zustandsänderung", "Adiabatische Zustandsänderung"], correctIndex: 3, explanation: "In einem perfekt isolierten Behälter findet kein Wärmeaustausch statt: Q = 0 → adiabatischer Prozess." }
    ]
  },
  {
    ...
    quiz: [
      { id: "q1", question: "Welcher Wärmetransportmechanismus kommt ohne Medium aus?", options: ["Konvektion", "Diffusion", "Wärmestrahlung", "Konduktion"], correctIndex: 2, explanation: "Wärmestrahlung (elektromagnetische Strahlung) kann sich auch im Vakuum ausbreiten – deshalb erreicht uns Sonnenwärme." },
      { id: "q2", question: "Was beschreibt das Fouriersche Gesetz?", options: ["Zusammenhang zwischen Temperatur und Druck", "Effizienz von Wärmekraftmaschinen", "Wärmestrom durch Leitung proportional zu ΔT und A, antiproportional zu d", "Strahlungsleistung des Schwarzen Körpers"], correctIndex: 2, explanation: "Fouriersches Gesetz: dQ/dt = λ·A·ΔT/d beschreibt den Wärmestrom durch Wärmeleitung." },
      { id: "q3", question: "Welches Material leitet Wärme am besten?", options: ["Luft", "Glas", "Wasser", "Kupfer"], correctIndex: 3, explanation: "Kupfer: λ ≈ 380 W/(m·K) – eines der besten Wärmeleitmaterialien unter Metallen. Luft ist mit λ ≈ 0,025 ein sehr schlechter Wärmeleiter." },
      { id: "q4", question: "Die Stefan-Boltzmann-Gleichung P = ε·σ·A·T⁴ zeigt: Wenn T verdoppelt wird, wie ändert sich P?", options: ["P verdoppelt sich", "P wächst um Faktor 8", "P wächst um Faktor 16", "P vervierfacht sich"], correctIndex: 2, explanation: "P ∝ T⁴: Verdopplung von T → P wächst um Faktor 2⁴ = 16." },
      { id: "q5", question: "Was ist natürliche Konvektion?", options: ["Strahlungsaustausch zwischen zwei Flächen", "Durch Temperaturunterschiede verursachte Dichteunterschiede treiben Strömung an", "Erzwungene Luftbewegung durch einen Ventilator", "Wärmeleitung durch ein flüssiges Medium"], correctIndex: 1, explanation: "Natürliche Konvektion: warme Flüssigkeit/Gas ist leichter und steigt auf; kaltes sinkt – treibende Kraft ist der Dichteunterschied." },
      { id: "yn1", type: "yesno", question: "Ist die Wärmeleitfähigkeit von Luft größer als die von Wasser?", correctAnswer: false, reasons: ["Nein, weil Wasser mit λ ≈ 0,6 W/(m·K) etwa 24-mal besser Wärme leitet als Luft mit λ ≈ 0,025 W/(m·K).", "Ja, weil Gase durch die freie Beweglichkeit ihrer Moleküle grundsätzlich besser Wärme leiten als Flüssigkeiten.", "Nein, weil Luft überhaupt keine Wärme leiten kann und λ_Luft = 0 ist.", "Ja, weil Luft eine geringere Dichte hat und Wärme dadurch schneller durch sie hindurch transportiert wird."], correctReasonIndex: 0, explanation: "Laut den angegebenen Werten hat Wasser (λ ≈ 0,6 W/(m·K)) eine deutlich höhere Wärmeleitfähigkeit als Luft (λ ≈ 0,025 W/(m·K)) – Luft ist ein schlechter Wärmeleiter, weshalb es z. B. als Dämmschicht in Doppelglasfenstern genutzt wird." },
      { id: "q6", question: "Warum ist Styropor ein guter Wärmeisolator?", options: ["Weil es sehr dicht ist", "Weil es viele Lufteinschlüsse hat (λ_Luft sehr klein)", "Weil es aus Plastik besteht", "Weil es Wärmestrahlung reflektiert"], correctIndex: 1, explanation: "Styropor enthält ca. 98% Luft in kleinen Poren. Da Luft einen sehr niedrigen λ-Wert hat (0,025 W/(m·K)) und die Poren Konvektion verhindern, ist Styropor ein ausgezeichneter Isolator." },
      { id: "q7", question: "Was ist ein Schwarzer Körper (Schwarzkörper)?", options: ["Ein Körper ohne Wärmekapazität", "Ein idealer Absorber und Emitter mit ε = 1", "Ein Körper mit schwarzer Oberfläche", "Ein Körper bei 0 K"], correctIndex: 1, explanation: "Der Schwarze Körper (ε = 1) absorbiert alle auftreffende Strahlung vollständig und emittiert maximal (Stefan-Boltzmann mit ε = 1)." },
      { id: "q8", question: "In einem Kühlschrank gibt der Kompressor Wärme an die Umgebung ab. Welcher Mechanismus kühlt den Innenraum?", options: ["Konvektion durch den Kältemittelkreislauf (erzwungen)", "Wärmeleitung durch die Wand", "Sublimation des Kältemittels", "Wärmestrahlung der Rückwand"], correctIndex: 0, explanation: "Das Kältemittel wird erzwungen durch Kompressor und Ventile zirkuliert (Konvektion), nimmt im Innenraum Wärme auf und gibt sie außen ab." },
      { id: "q9", question: "Eine Wand mit doppelter Dicke bei gleicher Fläche und ΔT: Wie ändert sich der Wärmestrom?", options: ["Er halbiert sich", "Er vervierfacht sich", "Er verdoppelt sich", "Er bleibt gleich"], correctIndex: 0, explanation: "dQ/dt = λ·A·ΔT/d: Bei doppelter Dicke d → halber Wärmestrom." },
      { id: "q10", question: "Warum ist eine Thermoskanne mit Vakuum und verspiegelter Innenwand so effizient?", options: ["Niederdruck kühlt den Inhalt", "Die Wände absorbieren alle Strahlung", "Vakuum leitet Wärme gut", "Vakuum verhindert Konduktion/Konvektion; Spiegel reduziert Strahlung"], correctIndex: 3, explanation: "Das Vakuum eliminiert Wärmeleitung und Konvektion. Die Verspiegelung reflektiert Infrarotstrahlung → minimaler Wärmeverlust durch alle drei Mechanismen." }
    ]
  }
];
