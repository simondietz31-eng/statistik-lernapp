const CHAPTER4_TOPICS = [
  {
    ...
    quiz: [
      { id: "q1", question: "Was unterscheidet die induktive von der deskriptiven Statistik?", options: ["Beide sind identisch", "Die induktive Statistik beschreibt nur die vorliegenden Daten", "Die induktive Statistik benötigt keine Wahrscheinlichkeitstheorie", "Die induktive Statistik schließt von einer Stichprobe auf eine unbekannte Grundgesamtheit"], correctIndex: 3, explanation: "Die induktive Statistik zieht einen wahrscheinlichkeitstheoretisch begründeten Schluss 'vom Teil aufs Ganze'." },
      { id: "q2", question: "Wann ist der Stichprobenmittelwert $\\bar{X}$ auch bei einer beliebig verteilten Grundgesamtheit näherungsweise normalverteilt?", options: ["Nie, das ist unmöglich", "Nur wenn die Grundgesamtheit bereits normalverteilt ist", "Für hinreichend großes $n$ (Faustregel $n \\geq 30$), nach dem Zentralen Grenzwertsatz", "Immer, unabhängig vom Stichprobenumfang"], correctIndex: 2, explanation: "Der Zentrale Grenzwertsatz garantiert die näherungsweise Normalverteilung für hinreichend große Stichproben." },
      { id: "q3", question: "Was bewirkt die Endlichkeitskorrektur beim Ziehen ohne Zurücklegen?", options: ["Sie verändert nur den Erwartungswert", "Sie macht die Verteilung schief", "Sie vergrößert die Varianz von $\\bar{X}$", "Sie verkleinert die Varianz von $\\bar{X}$ gegenüber dem Ziehen mit Zurücklegen"], correctIndex: 3, explanation: "Der Korrekturfaktor $(N-n)/(N-1)$ ist kleiner als 1 und reduziert damit die Varianz." },
      { id: "yn1", type: "yesno", question: "Vergrößert die Endlichkeitskorrektur beim Ziehen ohne Zurücklegen die Varianz von $\\bar{X}$ im Vergleich zum Ziehen mit Zurücklegen?", correctAnswer: false, reasons: [
        "Nein, weil der Korrekturfaktor $(N-n)/(N-1)$ stets kleiner als 1 ist und die Varianz verkleinert.",
        "Ja, weil beim Ziehen ohne Zurücklegen mehr Unsicherheit über die Grundgesamtheit besteht.",
        "Nein, weil die Endlichkeitskorrektur den Erwartungswert verändert, nicht die Varianz.",
        "Ja, weil der Korrekturfaktor mit wachsendem $n$ immer größer als 1 wird."
      ], correctReasonIndex: 0, explanation: "Der Faktor $(N-n)/(N-1)$ ist stets kleiner als 1, da beim Ziehen ohne Zurücklegen weniger Unsicherheit über die verbleibende Grundgesamtheit besteht - die Varianz von $\\bar{X}$ wird dadurch verkleinert, nicht vergrößert." },
      { id: "q4", question: "Eine Stichprobenfunktion ist...", options: ["identisch mit der Grundgesamtheit", "eine Funktion der Stichprobenvariablen und damit selbst eine Zufallsvariable", "nur bei normalverteilten Merkmalen definiert", "eine feste Zahl, die niemals variiert"], correctIndex: 1, explanation: "Da sie von den zufälligen Stichprobenvariablen abhängt, ist eine Stichprobenfunktion selbst zufällig." },
      { id: "q5", question: "Wenn eine Grundgesamtheit bereits exakt normalverteilt ist und mit Zurücklegen gezogen wird, gilt für $\\bar{X}$:", options: ["$\\bar{X}$ hat eine Chi-Quadrat-Verteilung", "$\\bar{X} \\sim N(\\mu, \\sigma^2/n)$ exakt, unabhängig vom Stichprobenumfang", "$\\bar{X}$ ist nur näherungsweise normalverteilt, egal wie groß $n$ ist", "$\\bar{X}$ ist nie normalverteilt"], correctIndex: 1, explanation: "Bei normalverteilter Grundgesamtheit und Ziehen mit Zurücklegen ist $\\bar{X}$ exakt (nicht nur näherungsweise) normalverteilt, auch für kleine $n$." },
      { id: "q6", question: "Der Stichprobenvektor $(X_1, \\dots, X_n)$ besteht aus...", options: ["den $n$ Stichprobenvariablen, die die einzelnen Ziehungen beschreiben", "der Grundgesamtheit selbst", "den $n$ konkreten Zahlenwerten einer Realisation", "einer einzigen Zufallsvariable, die $n$-mal beobachtet wird"], correctIndex: 0, explanation: "Der Stichprobenvektor fasst die $n$ Stichprobenvariablen $X_1,\\dots,X_n$ zusammen, deren Realisationen die konkrete Stichprobe ergeben." },
      { id: "q7", question: "Welche der folgenden Aussagen zur Verteilung von $\\bar{X}$ ist FALSCH?", options: ["Beim Ziehen ohne Zurücklegen ist die Endlichkeitskorrektur immer größer als 1", "Bei normalverteilter Grundgesamtheit und Ziehen mit Zurücklegen ist $\\bar{X}$ exakt normalverteilt", "Die Varianz von $\\bar{X}$ hängt vom Stichprobenumfang $n$ ab", "Nach dem Zentralen Grenzwertsatz ist $\\bar{X}$ für $n\\geq 30$ näherungsweise normalverteilt"], correctIndex: 0, explanation: "Die Endlichkeitskorrektur $(N-n)/(N-1)$ ist stets kleiner als 1, nicht größer - sie verkleinert die Varianz." },
      { id: "q8", question: "Für die Anwendung der Näherungsformel beim Ziehen ohne Zurücklegen ($n\\geq 30$ und $N \\geq 2n$) gilt zusätzlich zur Stichprobengröße die Bedingung:", options: ["$N$ muss exakt gleich $n$ sein", "Es gibt keine zusätzliche Bedingung an $N$", "Die Grundgesamtheit muss mindestens doppelt so groß sein wie die Stichprobe", "Die Grundgesamtheit muss kleiner als die Stichprobe sein"], correctIndex: 2, explanation: "Die Bedingung $N \\geq 2n$ verlangt, dass die Grundgesamtheit mindestens doppelt so groß ist wie der Stichprobenumfang." },
      { id: "q9", question: "Warum ist eine Stichprobenfunktion selbst eine Zufallsvariable und kein fester Wert?", options: ["Weil sie unabhängig von der gezogenen Stichprobe berechnet wird", "Weil sie per Definition immer den Wert 0 annehmen kann", "Weil die Grundgesamtheit zufällig ist", "Weil sie von den zufälligen Stichprobenvariablen abhängt, deren Realisationen von Stichprobe zu Stichprobe variieren"], correctIndex: 3, explanation: "Da unterschiedliche Stichproben zu unterschiedlichen Realisationen der Stichprobenvariablen führen, variiert auch der Wert der Stichprobenfunktion - sie ist daher selbst zufällig." }
    ]
  },
  {
    id: "schaetzverfahren",
    ...
    quiz: [
      { id: "q1", question: "Was ist der Hauptnachteil der reinen Punktschätzung gegenüber der Intervallschätzung?", options: ["Sie funktioniert nur bei kleinen Stichproben", "Es lässt sich keine Aussage über die Genauigkeit/Zuverlässigkeit der Schätzung treffen", "Sie benötigt immer eine Chi-Quadrat-Verteilung", "Sie ist rechnerisch aufwendiger"], correctIndex: 1, explanation: "Ein einzelner Punktschätzwert sagt nichts darüber aus, wie nah er am wahren Parameter liegt." },
      { id: "q2", question: "Was bedeutet Erwartungstreue einer Schätzfunktion?", options: ["Die Varianz der Schätzfunktion ist 0", "Die Schätzfunktion liefert immer den exakt richtigen Wert", "Der Erwartungswert der Schätzfunktion entspricht dem wahren Parameter", "Die Schätzfunktion ist unabhängig vom Stichprobenumfang"], correctIndex: 2, explanation: "Erwartungstreue bedeutet $E(\\hat{\\theta}) = \\theta$ - im Mittel über viele Stichproben trifft die Schätzung den wahren Wert." },
      { id: "q3", question: "Was passiert mit einem Konfidenzintervall, wenn man bei gleichem Stichprobenumfang das Konfidenzniveau von 95% auf 99% erhöht?", options: ["Das Intervall wird breiter", "Das Intervall bleibt gleich groß", "Das Konfidenzniveau hat keinen Einfluss auf die Intervallbreite", "Das Intervall wird enger"], correctIndex: 0, explanation: "Ein höheres Konfidenzniveau erfordert (bei gleichem $n$) ein breiteres Intervall - Genauigkeit und Sicherheit stehen im Zielkonflikt." },
      { id: "q4", question: "Wie verändert sich der maximale Schätzfehler eines Konfidenzintervalls, wenn der Stichprobenumfang $n$ vervierfacht wird (alles andere gleich)?", options: ["Er vervierfacht sich", "Er bleibt gleich", "Er verdoppelt sich", "Er halbiert sich"], correctIndex: 3, explanation: "Da der Standardfehler mit $\\sigma/\\sqrt{n}$ skaliert, führt eine Vervierfachung von $n$ zu einer Halbierung des Fehlers." },
      { id: "yn1", type: "yesno", question: "Führt eine Erhöhung des Konfidenzniveaus bei gleichbleibendem Stichprobenumfang zu einem engeren Konfidenzintervall?", correctAnswer: false, reasons: [
        "Nein, weil ein höheres Konfidenzniveau ein größeres Quantil $z_{1-\\alpha/2}$ und damit ein breiteres Intervall erfordert.",
        "Ja, weil höhere Sicherheit automatisch mit größerer Präzision einhergeht.",
        "Nein, weil das Konfidenzniveau die Intervallbreite überhaupt nicht beeinflusst.",
        "Ja, weil ein größeres $\\alpha$ stets ein engeres Intervall erzeugt."
      ], correctReasonIndex: 0, explanation: "Genauigkeit und Konfidenzniveau stehen im Zielkonflikt: ein höheres Konfidenzniveau verlangt ein größeres Quantil und führt bei gleichem $n$ zu einem breiteren, nicht engeren Intervall." },
      { id: "q5", question: "Ein einseitiges Konfidenzintervall verwendet man, wenn...", options: ["man nur an einer Ober- oder Untergrenze interessiert ist, nicht an beiden", "die Stichprobe zu klein ist", "$\\sigma$ unbekannt ist", "die Grundgesamtheit nicht normalverteilt ist"], correctIndex: 0, explanation: "Ein einseitiges Intervall passt, wenn nur interessiert, ob ein Mindest- oder Höchstwert über-/unterschritten wird." },
      { id: "q6", question: "Konsistenz einer Schätzfunktion bedeutet:", options: ["Sie ist immer erwartungstreu", "Sie hat konstante Varianz", "Mit wachsendem Stichprobenumfang nähert sie sich dem wahren Parameter an", "Sie funktioniert nur bei $n=1$"], correctIndex: 2, explanation: "Konsistenz beschreibt das Verhalten der Schätzfunktion für wachsendes $n$: sie konvergiert gegen den wahren Wert." },
      { id: "q7", question: "Was bedeutet Effizienz einer Schätzfunktion?", options: ["Sie ist die schnellste berechenbare Schätzfunktion", "Sie benötigt keinen Computer zur Berechnung", "Unter allen erwartungstreuen Schätzfunktionen gleichen Umfangs hat sie die kleinste Varianz", "Sie ist unabhängig von der Stichprobe"], correctIndex: 2, explanation: "Effizienz vergleicht die Varianz konkurrierender erwartungstreuer Schätzfunktionen - die effizienteste hat die kleinste Varianz." },
      { id: "q8", question: "Welche der folgenden Schritte gehört NICHT zur Konstruktion eines zweiseitigen Konfidenzintervalls für $\\mu$ bei bekanntem $\\sigma$?", options: ["Eine Chi-Quadrat-Teststatistik berechnen", "Das $(1-\\alpha/2)$-Quantil der Standardnormalverteilung bestimmen", "Den maximalen Schätzfehler berechnen", "Punktschätzung $\\bar{x}$ berechnen"], correctIndex: 0, explanation: "Die Chi-Quadrat-Teststatistik gehört zu Verteilungs- bzw. Unabhängigkeitstests, nicht zur Konstruktion eines Konfidenzintervalls für $\\mu$." },
      { id: "q9", question: "Was gibt das Signifikanzniveau $\\alpha$ im Kontext eines Konfidenzintervalls an?", options: ["Die exakte Breite des Konfidenzintervalls", "Den Stichprobenmittelwert", "Die Anzahl der gezogenen Stichproben", "Die Wahrscheinlichkeit, dass das Intervall den wahren Parameter NICHT überdeckt"], correctIndex: 3, explanation: "$\\alpha$ ist die Irrtumswahrscheinlichkeit - die Gegenwahrscheinlichkeit zum Konfidenzniveau $1-\\alpha$, mit der das Intervall den Parameter überdeckt." },
      { id: "q10", question: "Welche Aussage zum Zielkonflikt zwischen Genauigkeit und Konfidenzniveau ist korrekt?", options: ["Ein größerer Stichprobenumfang verschlechtert immer die Genauigkeit", "Ein höheres Konfidenzniveau bei gleichem $n$ führt zu einem breiteren Intervall - höhere Sicherheit kostet Genauigkeit", "Genauigkeit und Konfidenzniveau stehen in keinem Zusammenhang", "Ein schmaleres Intervall bei gleichem $n$ erfordert immer ein höheres Konfidenzniveau"], correctIndex: 1, explanation: "Bei festem $n$ erkauft man sich höhere Sicherheit (höheres Konfidenzniveau) durch ein breiteres, also weniger genaues Intervall." }
    ]
  },
  {
    ...
    quiz: [
      { id: "q1", question: "Der Fehler 1. Art (α-Fehler) tritt auf, wenn...", options: ["die Teststatistik negativ wird", "eine wahre $H_0$ fälschlich abgelehnt wird", "der Stichprobenumfang zu klein ist", "eine falsche $H_0$ fälschlich beibehalten wird"], correctIndex: 1, explanation: "Der α-Fehler beschreibt genau dieses Risiko und wird durch das Signifikanzniveau kontrolliert." },
      { id: "q2", question: "Welche Aussage über $H_0$ und $H_1$ ist korrekt?", options: ["$H_1$ wird nie verworfen", "$H_0$ ist immer die Behauptung, die bewiesen werden soll", "Üblicherweise formuliert man die zu beweisende Behauptung als $H_1$, da diese stärkere Evidenz benötigt", "$H_0$ und $H_1$ werden immer beide angenommen"], correctIndex: 2, explanation: "Da $H_0$ nur verworfen wird, wenn genügend Evidenz vorliegt, macht man die zu beweisende Aussage typischerweise zur Alternativhypothese." },
      { id: "q3", question: "Bei einem rechtsseitigen Test ($H_0: \\theta \\leq \\theta_0$ vs. $H_1: \\theta > \\theta_0$) berechnet sich der p-Wert als:", options: ["$2\\cdot P(T \\geq t_{\\text{beob}})$", "$P(H_0)$", "$P(T \\leq t_{\\text{beob}} \\mid H_0)$", "$P(T \\geq t_{\\text{beob}} \\mid H_0)$"], correctIndex: 3, explanation: "Beim rechtsseitigen Test interessiert die Wahrscheinlichkeit für mindestens so große Werte wie den beobachteten." },
      { id: "yn1", type: "yesno", question: "Wird die zu beweisende Behauptung in einem Hypothesentest üblicherweise als Nullhypothese $H_0$ formuliert?", correctAnswer: false, reasons: [
        "Nein, weil man die zu beweisende Behauptung üblicherweise als Alternativhypothese $H_1$ formuliert, da diese stärkere Evidenz erfordert.",
        "Ja, weil $H_0$ per Definition immer die Behauptung ist, die bewiesen werden soll.",
        "Nein, weil Hypothesentests grundsätzlich ohne Nullhypothese auskommen.",
        "Ja, weil $H_1$ ausschließlich für zweiseitige Tests verwendet wird."
      ], correctReasonIndex: 0, explanation: "Da $H_0$ nur bei ausreichend starker Evidenz verworfen wird, formuliert man die eigentlich zu beweisende Aussage typischerweise als Alternativhypothese $H_1$, nicht als Nullhypothese." },
      { id: "q4", question: "Wie lautet die Entscheidungsregel bei Verwendung des p-Werts?", options: ["Der p-Wert hat keinen Einfluss auf die Entscheidung", "$H_0$ verwerfen, wenn $p > \\alpha$", "$H_0$ verwerfen, wenn $p \\leq \\alpha$", "$H_0$ immer beibehalten, wenn $p$ existiert"], correctIndex: 2, explanation: "Ist der p-Wert kleiner oder gleich dem Signifikanzniveau, wird $H_0$ verworfen." },
      { id: "q5", question: "Ein Chi-Quadrat-Unabhängigkeitstest gehört zu welcher Testkategorie?", options: ["Verteilungstest", "Unabhängigkeitstest", "Punktschätzung", "Parametertest"], correctIndex: 1, explanation: "Er prüft explizit, ob zwei Merkmale unabhängig voneinander sind." },
      { id: "q6", question: "Die Sicherheitswahrscheinlichkeit $1-\\alpha$ ist...", options: ["die Wahrscheinlichkeit, eine falsche $H_0$ beizubehalten", "identisch mit dem p-Wert", "immer gleich 0,05", "die Wahrscheinlichkeit, eine wahre $H_0$ korrekt beizubehalten"], correctIndex: 3, explanation: "$1-\\alpha$ ergänzt das Signifikanzniveau zur Wahrscheinlichkeit einer korrekten Beibehaltungsentscheidung." },
      { id: "q7", question: "Ein linksseitiger Test wird verwendet, wenn die Alternativhypothese lautet:", options: ["$H_1: \\theta \\neq \\theta_0$", "$H_1: \\theta < \\theta_0$", "$H_1: \\theta > \\theta_0$", "$H_1: \\theta = \\theta_0$"], correctIndex: 1, explanation: "Ein linksseitiger Test prüft, ob der Parameter unterhalb des hypothetischen Werts liegt." },
      { id: "q8", question: "Ein Verteilungstest prüft eine Hypothese über...", options: ["die Unabhängigkeit zweier Merkmale", "einen einzelnen Verteilungsparameter wie den Mittelwert", "die Form der Verteilung eines Merkmals", "die Stichprobengröße"], correctIndex: 2, explanation: "Verteilungstests wie der Chi-Quadrat-Anpassungstest prüfen die angenommene Verteilungsform." },
      { id: "q9", question: "Der Fehler 2. Art (β-Fehler) tritt auf, wenn...", options: ["eine falsche $H_0$ fälschlich beibehalten wird", "der p-Wert exakt 0 ist", "das Signifikanzniveau zu hoch gewählt wurde", "eine wahre $H_0$ fälschlich abgelehnt wird"], correctIndex: 0, explanation: "Der β-Fehler ist im Gegensatz zum α-Fehler nicht direkt durch den Test kontrollierbar." },
      { id: "q10", question: "Bei einem zweiseitigen Test berechnet sich der p-Wert als:", options: ["$2 \\cdot \\min\\{P(T\\geq t_{\\text{beob}}), P(T\\leq t_{\\text{beob}})\\}$", "$P(T \\leq t_{\\text{beob}})$", "$1 - \\alpha$", "$P(T \\geq t_{\\text{beob}})$"], correctIndex: 0, explanation: "Beim zweiseitigen Test wird die kleinere der beiden einseitigen Wahrscheinlichkeiten verdoppelt." }
    ]
  },
  {
    id: "test-mittelwert",
    ...
    quiz: [
      { id: "q1", question: "Wie lautet die Teststatistik des z-Tests für den Mittelwert?", options: ["$z = \\frac{\\sigma}{\\sqrt{n}}$", "$z = \\bar{x} - \\mu_0$", "$z = n \\cdot \\bar{x}$", "$z = \\frac{\\bar{x}-\\mu_0}{\\sigma/\\sqrt{n}}$"], correctIndex: 3, explanation: "Die Teststatistik standardisiert die Abweichung des Stichprobenmittels vom hypothetischen Wert $\\mu_0$." },
      { id: "q2", question: "Bei einem zweiseitigen Test mit $\\alpha=0{,}05$ verwendet man als kritischen Wert...", options: ["$z_{0{,}05}$", "$z_{0{,}975}$", "$z_{0{,}95}$", "$z_{0{,}5}$"], correctIndex: 1, explanation: "Beim zweiseitigen Test teilt sich $\\alpha$ auf beide Seiten auf, daher braucht man das $(1-\\alpha/2)$-Quantil." },
      { id: "q3", question: "Wenn die berechnete Teststatistik $z$ betragsmäßig größer ist als der kritische Wert, bedeutet das:", options: ["$H_0$ wird verworfen, $H_1$ wird angenommen", "Es liegt automatisch ein Fehler 2. Art vor", "$H_0$ wird beibehalten", "Der Test ist ungültig"], correctIndex: 0, explanation: "Ein Wert jenseits des kritischen Werts liegt im Ablehnungsbereich, was zur Verwerfung von $H_0$ führt." },
      { id: "q4", question: "Warum sollte man das Testergebnis bei sehr kleinem Stichprobenumfang (z. B. $n=12$) vorsichtig interpretieren?", options: ["Kleine Stichproben sind nie ein Problem", "Weil die Schätzung des Mittelwerts bei kleinen Stichproben stärker vom Zufall beeinflusst und weniger robust ist", "Weil $\\sigma$ dann automatisch unbekannt ist", "Weil der z-Test bei kleinem $n$ grundsätzlich falsch ist"], correctIndex: 1, explanation: "Kleine Stichproben führen zu größerer Schwankung des Stichprobenmittelwerts und damit zu weniger belastbaren Testergebnissen." },
      { id: "q5", question: "Welche der folgenden Situationen erfordert einen einseitigen statt zweiseitigen Test?", options: ["Man möchte generell prüfen, ob sich $\\mu$ verändert hat", "Ein einseitiger Test ist nie sinnvoll", "Man möchte nur prüfen, ob $\\mu$ größer als ein Sollwert ist, nicht ob er allgemein abweicht", "Man hat keine konkrete Vermutung über die Richtung der Abweichung"], correctIndex: 2, explanation: "Ein einseitiger Test passt, wenn nur eine gerichtete Abweichung (größer oder kleiner) von Interesse ist." },
      { id: "q6", question: "Steigt oder sinkt der Standardfehler $\\sigma/\\sqrt{n}$, wenn der Stichprobenumfang $n$ erhöht wird?", options: ["Das hängt nur von $\\sigma$ ab, nicht von $n$", "Er bleibt unverändert", "Er sinkt", "Er steigt"], correctIndex: 2, explanation: "Da $n$ im Nenner unter der Wurzel steht, sinkt der Standardfehler mit wachsendem Stichprobenumfang." },
      { id: "q7", question: "Der z-Test für den Mittelwert setzt voraus, dass...", options: ["$\\mu_0$ unbekannt ist", "die Stichprobe kleiner als 10 sein muss", "kein Signifikanzniveau festgelegt werden muss", "die Grundgesamtheit (näherungsweise) normalverteilt und $\\sigma$ bekannt ist"], correctIndex: 3, explanation: "Der z-Test basiert auf einer (näherungsweise) normalverteilten Grundgesamtheit mit bekanntem $\\sigma$." },
      { id: "yn1", type: "yesno", question: "Sinkt der Standardfehler $\\sigma/\\sqrt{n}$ der Teststatistik, wenn der Stichprobenumfang $n$ erhöht wird?", correctAnswer: true, reasons: [
        "Ja, weil $n$ im Nenner unter der Wurzel steht und der Standardfehler damit kleiner wird.",
        "Nein, weil der Standardfehler ausschließlich von $\\sigma$ abhängt, nicht von $n$.",
        "Ja, weil eine größere Stichprobe automatisch $\\sigma$ verkleinert.",
        "Nein, weil $n$ nur den kritischen Wert, nicht den Standardfehler beeinflusst."
      ], correctReasonIndex: 0, explanation: "Da $n$ im Nenner unter der Wurzel steht, wird der Standardfehler $\\sigma/\\sqrt{n}$ mit wachsendem Stichprobenumfang kleiner - die Schätzung des Mittelwerts wird dadurch präziser." },
      { id: "q8", question: "Im Schrauben-Beispiel wurde $H_1: \\mu > 10$ gewählt. Das entspricht einem...", options: ["Test ohne Alternativhypothese", "rechtsseitigen Test", "linksseitigen Test", "zweiseitigen Test"], correctIndex: 1, explanation: "Da nur ein Überschreiten des Sollwerts (nach rechts) geprüft wird, handelt es sich um einen rechtsseitigen Test." },
      { id: "q9", question: "Was passiert mit dem kritischen Wert, wenn $\\alpha$ von 0,05 auf 0,01 gesenkt wird (bei sonst gleichem Test)?", options: ["Der kritische Wert wird größer (Ablehnungsbereich schrumpft), es wird schwerer $H_0$ zu verwerfen", "Der kritische Wert wird kleiner (Ablehnungsbereich schrumpft)", "Der kritische Wert bleibt unverändert", "Der Test wird ungültig"], correctIndex: 0, explanation: "Ein kleineres Signifikanzniveau verlangt stärkere Evidenz, der kritische Wert rückt weiter von der Mitte weg." }
    ]
  },
  {
    ...
    quiz: [
      { id: "q1", question: "Was prüft der Chi-Quadrat-Verteilungstest (Anpassungstest)?", options: ["Ob der Mittelwert einem Sollwert entspricht", "Ob die Varianz gleich 0 ist", "Ob eine angenommene theoretische Verteilung zu den beobachteten Daten passt", "Ob zwei Merkmale unabhängig sind"], correctIndex: 2, explanation: "Der Anpassungstest vergleicht beobachtete mit theoretisch erwarteten Häufigkeiten unter einer angenommenen Verteilung." },
      { id: "q2", question: "Wie viele Freiheitsgrade hat der Chi-Quadrat-Unabhängigkeitstest bei $v$ Kategorien des einen und $w$ Kategorien des anderen Merkmals?", options: ["$v + w$", "$v \\cdot w$", "$v - w$", "$(v-1)(w-1)$"], correctIndex: 3, explanation: "Die Freiheitsgrade des Unabhängigkeitstests ergeben sich als Produkt der um je 1 reduzierten Kategorienanzahlen." },
      { id: "q3", question: "Ein großer Chi-Quadrat-Wert (weit im Ablehnungsbereich) deutet darauf hin, dass...", options: ["die beobachteten und erwarteten Häufigkeiten stark voneinander abweichen", "der Stichprobenumfang zu klein war", "die beobachteten und erwarteten Häufigkeiten sehr gut übereinstimmen", "$H_0$ auf jeden Fall wahr ist"], correctIndex: 0, explanation: "Große Abweichungen zwischen beobachteten und erwarteten Häufigkeiten führen zu einem großen $\\chi^2$-Wert und damit eher zur Ablehnung von $H_0$." },
      { id: "q4", question: "Wie werden die erwarteten Häufigkeiten $n_{ij}'$ beim Unabhängigkeitstest berechnet?", options: ["$n_{ij}' = n_{i\\cdot} \\cdot n_{\\cdot j}$", "$n_{ij}' = \\frac{n_{i\\cdot}\\cdot n_{\\cdot j}}{n}$", "$n_{ij}' = n_{i\\cdot} + n_{\\cdot j}$", "$n_{ij}' = n$"], correctIndex: 1, explanation: "Unter der Unabhängigkeitsannahme ergibt sich die erwartete gemeinsame Häufigkeit als Produkt der Randhäufigkeiten geteilt durch den Gesamtumfang." },
      { id: "q5", question: "Liegt der berechnete $\\chi^2$-Wert im Beibehaltungsbereich $[0, \\chi^2_{1-\\alpha;v-1}]$, bedeutet das:", options: ["$H_0$ wird beibehalten - die Abweichung ist nicht signifikant groß", "Der Test ist ungültig", "Es liegt garantiert kein Fehler vor", "$H_0$ wird verworfen"], correctIndex: 0, explanation: "Innerhalb des Beibehaltungsbereichs gibt es keine ausreichende Evidenz, um die Nullhypothese zu verwerfen." },
      { id: "yn1", type: "yesno", question: "Deutet ein kleiner Chi-Quadrat-Wert im Beibehaltungsbereich auf eine gute Übereinstimmung zwischen beobachteten und erwarteten Häufigkeiten hin?", correctAnswer: true, reasons: [
        "Ja, weil kleine Differenzen $(n_i-n_i')$ zu einem kleinen $\\chi^2$-Wert führen, der die Nullhypothese nicht verwirft.",
        "Nein, weil ein kleiner $\\chi^2$-Wert stets auf einen zu geringen Stichprobenumfang hinweist.",
        "Ja, weil ein kleiner $\\chi^2$-Wert bedeutet, dass gar keine Daten erhoben wurden.",
        "Nein, weil die Größe des $\\chi^2$-Werts unabhängig von der Übereinstimmung der Häufigkeiten ist."
      ], correctReasonIndex: 0, explanation: "Da $\\chi^2$ die quadrierten, normierten Abweichungen zwischen beobachteten und erwarteten Häufigkeiten summiert, führt eine gute Übereinstimmung zu einem kleinen Wert im Beibehaltungsbereich - die angenommene Verteilung bzw. Unabhängigkeit wird dann nicht verworfen." },
      { id: "q6", question: "Welche Art von Merkmalen eignet sich typischerweise für einen Chi-Quadrat-Unabhängigkeitstest?", options: ["Kategoriale (z. B. nominal- oder ordinalskalierte) Merkmale", "Nur ein einzelnes metrisches Merkmal", "Merkmale, die bereits als unabhängig bekannt sind", "Nur stetige, normalverteilte Merkmale"], correctIndex: 0, explanation: "Der Test arbeitet mit Häufigkeitstabellen (Kontingenztafeln) für kategoriale Merkmale." },
      { id: "q7", question: "Beim Chi-Quadrat-Verteilungstest bezeichnet $n_i'$...", options: ["das Signifikanzniveau", "die unter $H_0$ erwartete (theoretische) Häufigkeit", "die Anzahl der Freiheitsgrade", "die beobachtete Häufigkeit"], correctIndex: 1, explanation: "$n_i$ ist die beobachtete, $n_i'$ die theoretisch erwartete Häufigkeit unter der angenommenen Verteilung." },
      { id: "q8", question: "Der Chi-Quadrat-Verteilungstest hat wie viele Freiheitsgrade bei $v$ unterschiedlichen Merkmalswerten?", options: ["$v + 1$", "$v^2$", "$v$", "$v - 1$"], correctIndex: 3, explanation: "Die Freiheitsgrade betragen $v-1$, da ein Freiheitsgrad durch die Normierungsbedingung verloren geht." },
      { id: "q9", question: "Im Beispiel zur Pausenregelung war $\\chi^2 \\approx 1{,}40$ deutlich kleiner als der kritische Wert 5,99. Das bedeutet:", options: ["Die Merkmale sind stark voneinander abhängig", "Es müssen mehr Kategorien gebildet werden", "Es gibt keine ausreichende Evidenz gegen die Unabhängigkeit der Merkmale", "Der Test war fehlerhaft"], correctIndex: 2, explanation: "Ein kleiner Chi-Quadrat-Wert im Beibehaltungsbereich spricht nicht gegen die Unabhängigkeitsannahme." },
      { id: "q10", question: "Was unterscheidet den Chi-Quadrat-Verteilungstest grundsätzlich vom z-Test für den Mittelwert?", options: ["Es gibt keinen Unterschied", "Der Chi-Quadrat-Test benötigt kein Signifikanzniveau", "Der Chi-Quadrat-Test prüft eine Verteilungsform bzw. Unabhängigkeit statt eines einzelnen Parameters wie beim z-Test", "Der z-Test kann keine Nullhypothese haben"], correctIndex: 2, explanation: "Der z-Test ist ein Parametertest, während Chi-Quadrat-Tests Verteilungsform oder Unabhängigkeit prüfen." }
    ]
  }
];
