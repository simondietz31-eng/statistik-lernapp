const NATWISS_CHAPTER11_TOPICS = [
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Verdoppelt sich der Widerstand eines Drahtes, wenn sowohl seine Länge als auch sein Durchmesser verdoppelt werden?", correctAnswer: false, reasons: ["Nein, weil die Querschnittsfläche A ∝ d² vervierfacht, R aber nur linear mit L wächst – insgesamt halbiert sich R = ρL/A.", "Ja, weil sich Länge und Durchmesser jeweils verdoppeln und sich beide Effekte linear addieren, sodass sich der Gesamtwiderstand am Ende exakt verdoppelt.", "Nein, weil der Widerstand bei gleichzeitiger Skalierung von Länge und Durchmesser stets exakt konstant bleibt, unabhängig vom Skalierungsfaktor.", "Ja, weil der spezifische Widerstand ρ als Materialkonstante bei größeren Drahtabmessungen automatisch entsprechend proportional ansteigt."], correctReasonIndex: 0, explanation: "R = ρL/A mit A = π(d/2)²; bei L' = 2L und d' = 2d wird A' = 4A, sodass R' = ρ·2L/(4A) = R/2 – der Widerstand halbiert sich, weil die quadratische Zunahme der Querschnittsfläche die lineare Zunahme der Länge überkompensiert." },
  ...
  },
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Ist der Gesamtwiderstand einer Parallelschaltung stets größer als der größte beteiligte Einzelwiderstand?", correctAnswer: false, reasons: ["Nein, weil 1/R_p = 1/R₁ + 1/R₂ + … stets einen Gesamtwiderstand liefert, der kleiner ist als der kleinste Einzelwiderstand, nicht größer als der größte.", "Ja, weil sich bei einer Parallelschaltung die einzelnen Widerstandswerte einfach addieren, ganz ähnlich wie es auch bei einer gewöhnlichen Reihenschaltung der Fall ist.", "Nein, weil der Gesamtwiderstand einer Parallelschaltung unabhängig von den Einzelwerten immer exakt gleich dem größten beteiligten Einzelwiderstand ist.", "Ja, weil zusätzliche parallele Widerstände dem Strom insgesamt mehr Widerstand entgegensetzen und den Gesamtwiderstand dadurch stetig erhöhen."], correctReasonIndex: 0, explanation: "Bei einer Parallelschaltung addieren sich die Kehrwerte der Widerstände, wodurch R_p stets kleiner ist als der kleinste Einzelwiderstand – jeder zusätzliche parallele Pfad bietet dem Strom eine weitere Möglichkeit zu fließen und senkt den Gesamtwiderstand." },
  ...
  },
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Ist die Joulesche Wärme, die in einem Widerstand entsteht, unabhängig von der Stromrichtung?", correctAnswer: true, reasons: ["Ja, weil Q_W = I²·R·t vom Quadrat des Stroms abhängt; I² ist stets positiv, daher spielt die Stromrichtung keine Rolle für die Wärme.", "Nein, weil bei negativer Stromrichtung auch die erzeugte Wärme negativ wird und dem System dabei Energie entzieht.", "Ja, weil Widerstände laut Definition grundsätzlich niemals Wärme erzeugen können, ganz unabhängig davon, wie groß der fließende Strom tatsächlich ist.", "Nein, weil nach gängiger Annahme nur bei Gleichstrom tatsächlich Wärme entsteht, bei Wechselstrom mit ständigen Richtungswechseln jedoch nicht."], correctReasonIndex: 0, explanation: "Da Q_W = I²·R·t quadratisch vom Strom abhängt, liefert sowohl positiver als auch negativer Strom denselben (positiven) Wert für I² – die erzeugte Wärme ist daher unabhängig von der Stromrichtung, weshalb auch Wechselstrom ebenso Wärme erzeugt wie Gleichstrom." },
