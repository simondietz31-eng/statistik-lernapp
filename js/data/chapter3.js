const CHAPTER3_TOPICS = [
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Muss der Ereignisraum $\\Omega$ eines Zufallsvorgangs immer diskret sein?", correctAnswer: false, reasons: [
        "Nein, weil es auch stetige Ereignisräume mit überabzählbar vielen Ausgängen gibt, z. B. bei einer Wartezeit.",
        "Ja, weil die Kolmogorow-Axiome ausschließlich für diskrete Ereignisräume formuliert sind.",
        "Nein, weil Ereignisräume grundsätzlich nur ein einziges Elementarereignis enthalten.",
        "Ja, weil ein Zufallsvorgang per Definition endlich viele Ausgänge haben muss."
      ], correctReasonIndex: 0, explanation: "Man unterscheidet diskrete Ereignisräume (endlich oder abzählbar unendlich, z. B. Würfelwurf) und stetige Ereignisräume (überabzählbar viele Ausgänge, z. B. Wartezeit) - Diskretheit ist also keine notwendige Eigenschaft von $\\Omega$." },
  ...
  },
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Setzt die klassische (Laplace-) Methode voraus, dass alle Elementarereignisse gleich wahrscheinlich sind?", correctAnswer: true, reasons: [
        "Ja, weil die Formel $P(A)=|A|/|\\Omega|$ nur bei Gleichwahrscheinlichkeit gültig ist.",
        "Nein, weil die klassische Methode für beliebig verteilte Elementarereignisse funktioniert.",
        "Ja, weil sonst kein Zufallsvorgang wiederholbar wäre.",
        "Nein, weil Gleichwahrscheinlichkeit nur bei der subjektiven Methode gefordert wird."
      ], correctReasonIndex: 0, explanation: "Die Laplace-Formel zählt günstige durch mögliche Fälle und liefert nur dann korrekte Wahrscheinlichkeiten, wenn alle Elementarereignisse gleich wahrscheinlich sind." },
  ...
  },
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Gilt der vereinfachte Additionssatz $P(A \\cup B) = P(A) + P(B)$ auch, wenn sich $A$ und $B$ überschneiden?", correctAnswer: false, reasons: [
        "Nein, weil dann die Schnittmenge $P(A \\cap B)$ doppelt gezählt würde und abgezogen werden muss.",
        "Ja, weil der Additionssatz unabhängig von einer Überschneidung immer gilt.",
        "Nein, weil sich überschneidende Ereignisse grundsätzlich die Wahrscheinlichkeit 0 haben.",
        "Ja, weil sich Doppelzählungen bei Wahrscheinlichkeiten automatisch aufheben."
      ], correctReasonIndex: 0, explanation: "Die vereinfachte Formel gilt nur für disjunkte (sich ausschließende) Ereignisse; überschneiden sich $A$ und $B$, muss man $P(A\\cap B)$ gemäß dem allgemeinen Additionssatz abziehen, um die Doppelzählung zu korrigieren." },
  ...
  },
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Kann der Erwartungswert einer diskreten Zufallsvariable ein Wert sein, den die Zufallsvariable selbst gar nicht annehmen kann?", correctAnswer: true, reasons: [
        "Ja, weil der Erwartungswert ein gewichteter Durchschnitt über alle möglichen Werte ist, kein tatsächlich auftretender Einzelwert.",
        "Nein, weil der Erwartungswert stets mit einem der möglichen Werte $x$ übereinstimmen muss.",
        "Ja, weil diskrete Zufallsvariablen per Definition keinen Erwartungswert besitzen.",
        "Nein, weil sich der Erwartungswert immer auf den Modus reduziert."
      ], correctReasonIndex: 0, explanation: "Wie beim zweifachen Würfelwurf mit $E(X)=1/3$ Sechsen zeigt sich: der Erwartungswert ist ein über viele Wiederholungen gemittelter theoretischer Wert und muss selbst kein möglicher Einzelwert der Zufallsvariable sein." },
  ...
  },
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Ist die Wahrscheinlichkeit, dass eine stetige Zufallsvariable exakt einen bestimmten Wert $x$ annimmt, immer gleich null?", correctAnswer: true, reasons: [
        "Ja, weil bei überabzählbar vielen möglichen Werten kein einzelner Punkt eine positive Fläche unter der Dichte besitzt.",
        "Nein, weil $P(X=x)$ immer der Dichte $f(x)$ an dieser Stelle entspricht.",
        "Ja, weil stetige Zufallsvariablen grundsätzlich keine Dichtefunktion besitzen.",
        "Nein, weil nur bei der Gleichverteilung $P(X=x)=0$ gilt."
      ], correctReasonIndex: 0, explanation: "Da ein einzelner Punkt keine Fläche unter der Dichtefunktion einschließt, gilt für jede stetige Zufallsvariable $P(X=x)=0$ - nur Intervalle können eine positive Wahrscheinlichkeit haben." },
  ...
  },
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Sind bei der Poissonverteilung Erwartungswert und Varianz stets gleich groß?", correctAnswer: true, reasons: [
        "Ja, weil bei $X \\sim \\text{Po}(\\lambda)$ sowohl $E(X)$ als auch $\\text{Var}(X)$ gleich $\\lambda$ sind.",
        "Nein, weil die Varianz bei der Poissonverteilung immer doppelt so groß ist wie der Erwartungswert.",
        "Ja, weil das bei jeder diskreten Verteilung grundsätzlich so ist.",
        "Nein, weil $E(X)$ und $\\text{Var}(X)$ von zwei unabhängigen Parametern abhängen."
      ], correctReasonIndex: 0, explanation: "Eine charakteristische Eigenschaft der Poissonverteilung ist, dass Erwartungswert und Varianz beide exakt dem Parameter $\\lambda$ entsprechen." },
  ...
  },
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Wird die Form der Normalverteilungsdichte vollständig durch genau zwei Parameter, Erwartungswert und Varianz, festgelegt?", correctAnswer: true, reasons: [
        "Ja, weil $\\mu$ die Lage und $\\sigma^2$ die Streuung der glockenförmigen Dichte bestimmen.",
        "Nein, weil zusätzlich noch der Schiefekoeffizient benötigt wird.",
        "Ja, weil jede Normalverteilung automatisch $\\mu=0$ und $\\sigma=1$ hat.",
        "Nein, weil die Normalverteilung durch unendlich viele unabhängige Parameter beschrieben wird."
      ], correctReasonIndex: 0, explanation: "Die Dichte $f(x)$ von $N(\\mu,\\sigma^2)$ hängt formelmäßig nur von $\\mu$ (Lage) und $\\sigma^2$ (Streuung) ab - diese beiden Parameter legen Lage und Form der Glockenkurve vollständig fest." },
  ...
  },
  {
    ...
    quiz: [
      ...
      { id: "yn1", type: "yesno", question: "Kann eine Chi-Quadrat-verteilte Zufallsvariable negative Werte annehmen?", correctAnswer: false, reasons: [
        "Nein, weil sie als Summe von Quadraten standardnormalverteilter Zufallsvariablen definiert ist und Quadrate nie negativ sind.",
        "Ja, weil $\\chi^2$-verteilte Zufallsvariablen symmetrisch um 0 verteilt sind.",
        "Nein, weil sie nur Werte zwischen -1 und 1 annehmen kann.",
        "Ja, weil bei hoher Anzahl an Freiheitsgraden auch negative Ausprägungen möglich werden."
      ], correctReasonIndex: 0, explanation: "Da $X = \\sum Z_i^2$ eine Summe von Quadraten ist und Quadrate stets $\\geq 0$ sind, kann die Chi-Quadrat-Verteilung ausschließlich nicht-negative Werte annehmen." },
